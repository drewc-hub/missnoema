...
function GateCard({ nextUrl }: { nextUrl: string }) {
  return (
...
export async function CompanionDetailView({
  ...
  // If this is an adult route, show gate UI instead of throwing (Option A)
  if (forceAdult) {
    if (!user) {
      const nextUrl = `/adult/companions/${slug}`;
      return (
        <main className="mx-auto max-w-2xl space-y-4">
          <Card>
            <CardHeader
              title="Login required"
              subtitle="Please login to access the adult library."
              right={<Badge>Web-only</Badge>}
            />
    ...
    if (!isAdultAllowed(user)) {
      const nextUrl = `/adult/companions/${slug}`;
      return <GateCard nextUrl={nextUrl} />;
    }
  }
  ...
  // If an ADULT companion is accessed from SAFE route, show gate card (nice UX)
      ...
      return (
        <main className="mx-auto max-w-2xl space-y-4">
          <Card>
            <CardHeader
              title="Login required"
              subtitle="This companion is 18+ and web-only."
              right={<Badge>Web-only</Badge>}
            />
            <CardBody className="space-y-3">
              <a href={`/login?next=${encodeURIComponent(nextUrl)}`}>
  ...
  const imageAssets = (assets as Asset[]).filter((a) => a.type === "IMAGE");
            ...
            <CardBody className="space-y-4">
              ...
              <div className="flex items-center justify-between gap-3">
                <div className="text-sm text-zinc-500">
                  Showing <span className="text-zinc-200">{assets.length}</span>{" "}
