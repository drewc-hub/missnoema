...
export function MediaGenPanel({
    ...
    const [focalPoint, setFocalPoint] = useState<{ x: number; y: number }>({ x: 50, y: 18 });
    ...
    const CROP_WIDTH_PERCENT = 44;
    const CROP_ASPECT = 188 / 330;
    ...
    function getPctFromEvent(
        ...
        return {
            x: Math.round(clamp(rawX, minX, maxX)),
            y: Math.round(clamp(rawY, minY, maxY)),
        };
    }
    ...
    function handlePickerMouseUp(e: React.MouseEvent<HTMLDivElement>) {
        ...
        const pt = getPctFromEvent(e);
        const finalPoint = pt ?? focalPoint;

        setFocalPoint(finalPoint);
        void saveFocalPoint(finalPoint.x, finalPoint.y);
    }
                ...
                <div className="space-y-4">
                    ...
                    <div className="space-y-2">
                        ...
                        <textarea
                            ...
                            className={`w-full resize-y rounded-xl border bg-zinc-950 px-3 py-2 text-sm text-zinc-100 placeholder:text-zinc-500 transition-[min-height] focus:outline-none focus:ring-2 focus:ring-white/20 ${prompt.length > 1500 ? "border-amber-700/60" : "border-zinc-800"
