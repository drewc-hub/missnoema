...
const worldTabCopy = {
  "New World": {
    ...
    body: "Set the shared setting, table capacity, and opening premise from the world lobby.",
    href: "/worlds",
    cta: "Open world creator",
  },
  "New Story": {
    title: "Draft a story arc",
    body: "Use the selected world setting plus the active character scenario as the seed for your next arc.",
    href: "/roleplay",
    cta: "Open roleplay forum",
  },
  "New Scene": {
    title: "Start a scene",
...
export function WorldStudioClient({
  worlds,
  companions,
  ageVerified,
}: WorldStudioClientProps) {
  const [workspaceTab, setWorkspaceTab] =
    useState<(typeof workspaceTabs)[number]>("New World");
  const [rosterTab, setRosterTab] =
    useState<(typeof rosterTabs)[number]>("New");
  const [builderTab, setBuilderTab] =
    useState<(typeof builderTabs)[number]>("Lorebook");
  ...
  const initialDraft = useMemo(
    () => getCharacterDraft(selectedCompanion),
    [selectedCompanion],
  );
  ...
  function updateSlider(key: keyof typeof draft.sliders, value: number) {
    setDraft((current) => ({
      ...current,
      sliders: { ...current.sliders, [key]: value },
    }));
  }
  ...
  async function runFieldGeneration(
    field: "personality" | "backstory",
    label: string,
  ) {
    if (!selectedCompanion || busy) return;
    setBusy(true);
    ...
    try {
      const res = await fetch("/api/studio/character", {
        ...
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          mode: field === "personality" ? "rewrite" : "expand",
          field,
      ...
      if (data.ok && typeof data.text === "string") {
        ...
        flash(data.error ?? `${label} failed.`, false);
      }
    } catch {
      flash(`${label} failed.`, false);
    } finally {
      setBusy(false);
    }
  }
  ...
  async function handleRewrite() {
    await runFieldGeneration("personality", "Rewriting personality");
  }

  async function handleExpand() {
    await runFieldGeneration("backstory", "Expanding backstory");
  }
  ...
  async function handleImageGenerate() {
    ...
    const prompt =
      imagePrompt || draft.appearance || selectedCompanion.description;
    if (!prompt) {
      flash("Add an appearance description or prompt first.", false);
      return;
    }
  ...
  return (
    <div className="rounded-lg border border-cyan-900/40 bg-[linear-gradient(180deg,#06133f_0%,#071c50_46%,#0a1024_100%)] p-3 text-zinc-100 shadow-2xl shadow-blue-950/40 sm:p-4">
      <div
        className={cn(
          "grid gap-4",
          editMode
          ...
          <header className="flex flex-wrap items-center justify-between gap-3">
            <div>
              <div className="text-lg font-semibold text-white">
                {selectedWorld?.name ?? "No world selected"}
              </div>
              <div className="mt-1 text-sm text-cyan-100/75">
                {workspaceTab} ·{" "}
                {selectedWorld?.summary ?? "Create or select a world to begin."}
              </div>
            </div>
            <Badge>
              {selectedWorld
                ? `${selectedWorld.messageCount} turns`
                : "World builder"}
            </Badge>
          </header>
          ...
          <div className="mt-4 grid gap-3 lg:grid-cols-3">
            <div className="rounded-lg border border-cyan-950/50 bg-zinc-950/55 p-4 lg:col-span-2">
              <div className="flex items-center gap-2 text-sm font-semibold text-white">
                ...
                {activeWorldCopy.title}
              </div>
              <p className="mt-2 line-clamp-3 text-sm leading-6 text-cyan-100/75">
                {workspaceTab === "New World"
                  ? activeWorldCopy.body
                  : selectedWorld?.setting || activeWorldCopy.body}
              </p>
              <a
                href={
                  workspaceTab === "New Scene"
                    ? sceneHref
                    : activeWorldCopy.href
                }
                className="mt-4 inline-flex h-9 items-center rounded-lg border border-cyan-300/40 bg-zinc-950/70 px-3 text-sm font-semibold text-white transition hover:bg-zinc-900"
              >
                {activeWorldCopy.cta}
              </a>
            </div>
            ...
            <div className="rounded-lg border border-cyan-950/50 bg-zinc-950/45 p-4">
              ...
              <div className="mt-1 text-xs leading-5 text-cyan-100/70">
                {builderTab} is selected for{" "}
                {selectedCompanion?.name ?? "the current character"}.
              </div>
            </div>
          </div>
        </section>
        ...
        >
          <header className="flex items-center justify-between gap-3">
            <div>
              ...
              <div className="text-xs text-cyan-100/70">
                World, story, and scene inventory
              </div>
            </div>
          ...
          <div className="mt-4 min-h-[380px] rounded-lg border border-cyan-950/50 bg-zinc-950/55 p-3">
            ...
            {rosterTab === "New" ? (
              <a
                href="/companions/new"
                className="mt-3 flex h-10 items-center justify-center rounded-lg border border-cyan-300/40 bg-zinc-950/60 text-sm font-semibold text-white transition hover:bg-zinc-900"
              >
              ...
              <button
                ...
                className="mt-3 flex h-10 w-full items-center justify-center gap-2 rounded-lg border border-cyan-300/40 bg-zinc-950/60 text-sm font-semibold text-white transition hover:bg-zinc-900 disabled:cursor-not-allowed disabled:opacity-50"
              >
                <Wand2 className="h-4 w-4" />
                Generate into selected card
              </button>
            )}
        ...
        >
          <header className="flex flex-wrap items-center justify-between gap-3">
            <div>
              ...
              <div className="text-xs text-cyan-100/70">
                {selectedCompanion
                  ? `Editing ${selectedCompanion.name}`
                  : "Create a SillyTavern V2-style card"}
              </div>
            </div>
          ...
          >
            <div className="flex items-center justify-between border-b border-cyan-900/50 px-4 py-3">
              <div>
                <div className="text-sm font-semibold text-white">
                  {selectedCompanion
                    ? `${selectedCompanion.name}'s Image`
                    : "Companion Image"}
                </div>
                <div className="text-xs text-cyan-100/60">
                  Upload, generate, or edit
                </div>
              </div>
              <button
                type="button"
              ...
              {imagePanelTab === "generate" && (
                <div className="flex h-full flex-col gap-4">
                  ...
                  <div className="rounded-lg border border-cyan-950/50 bg-zinc-950/40 p-3 text-xs text-cyan-100/60">
                    The appearance field will be used as a base prompt if left
                    blank.
                  </div>
                  <button
                    type="button"
          ...
          <div className="mt-4 min-h-0 flex-1 overflow-y-auto rounded-lg border border-cyan-950/50 bg-zinc-950/55 p-4">
            ...
            <div className="grid gap-4 xl:grid-cols-2">
              ...
              <Field
                ...
                rows={4}
                onChange={(value) => updateDraft("scenario", value)}
              />
              <Field
                label="First Message"
                value={draft.firstMessage}
                rows={4}
                onChange={(value) => updateDraft("firstMessage", value)}
              />
              <Field
                label="Example Dialogue"
                value={draft.exampleDialogue}
                rows={5}
                onChange={(value) => updateDraft("exampleDialogue", value)}
              />
              <Field
                label="System Prompt"
              ...
              <Field
                ...
                rows={4}
                onChange={(value) =>
                  updateDraft("postHistoryInstructions", value)
                }
              />
              <Field
                label="Creator Notes"
                ...
                rows={4}
                onChange={(value) => updateDraft("creatorNotes", value)}
              />
            </div>
            ...
            <div className="mt-6 rounded-lg border border-cyan-950/50 bg-blue-950/30 p-4">
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                <SliderField
                  label="Jealousy"
                  value={draft.sliders.jealousy}
                  onChange={(v) => updateSlider("jealousy", v)}
                />
                <SliderField
                  label="Dominance Style"
                  value={draft.sliders.dominance}
                  onChange={(v) => updateSlider("dominance", v)}
                />
                <SliderField
                  label="Affection Style"
                  value={draft.sliders.affection}
                  onChange={(v) => updateSlider("affection", v)}
                />
                <SliderField
                  label="Empathy"
                  value={draft.sliders.empathy}
                  onChange={(v) => updateSlider("empathy", v)}
                />
              </div>
            </div>
            ...
            <div className="mt-4 rounded-lg border border-cyan-950/50 bg-blue-950/40 p-3">
              <div className="flex items-center gap-2 text-sm font-semibold text-white">
                ...
                {builderTab === "Scenario Wizard" ? (
                  <Workflow className="h-4 w-4" />
                ) : null}
                {builderTab}
              </div>
              <div className="mt-2 text-xs leading-5 text-cyan-100/75">
                {builderTab === "Lorebook" ? (
                  <p>
                    {draft.backstory ||
                      draft.scenario ||
                      "Add backstory and scenario details to seed lorebook entries."}
                  </p>
                ) : null}
                {builderTab === "Memory" ? (
                  <p>
                    {draft.firstMessage ||
                      "The first message becomes the session memory seed for this character."}
                  </p>
                ) : null}
                {builderTab === "Personas" ? (
                  <p>
                    {draft.personality ||
                      "Describe personality to shape how this character behaves in scenes."}
                  </p>
                ) : null}
                {builderTab === "Data" ? (
                  <div className="grid gap-2 sm:grid-cols-2">
                    <span>Name: {draft.name || "Untitled"}</span>
                    <span>Rating: {draft.contentRating}</span>
                    <span>Affection: {draft.sliders.affection}</span>
                    <span>Empathy: {draft.sliders.empathy}</span>
                  </div>
                ) : null}
                {builderTab === "Scenario Wizard" ? (
                  <p>
                    {draft.scenario ||
                      ...
                      "Select a world or write a scenario to start building the opening scene."}
                  </p>
                ) : null}
              </div>
            </div>
