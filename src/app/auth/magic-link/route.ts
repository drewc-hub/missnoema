...
export async function GET(req: NextRequest) {
  ...
  if (!code) {
    return NextResponse.redirect(new URL("/login?error=no-code", url.origin), 303);
  }
  ...
  if (exchangeError) {
    console.error("[magic-link] exchange failed", exchangeError.message);
    const res = NextResponse.redirect(new URL("/login?error=exchange-failed", url.origin), 303);
  ...
  if (userError || !user) {
    const res = NextResponse.redirect(new URL("/login?error=no-user", url.origin), 303);
    return applySupabaseCookies(res, cookiesToSet);
  }
