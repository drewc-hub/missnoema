    ...
    <main className="space-y-6 text-zinc-100">
      <section className="overflow-hidden rounded-lg border border-zinc-800 bg-black">
        <div className="relative min-h-[360px]">
          ...
          <div className="relative flex min-h-[360px] max-w-3xl flex-col justify-center px-5 py-10 sm:px-8">
            <div className="inline-flex w-fit items-center gap-2 rounded-full border border-fuchsia-500/30 bg-fuchsia-500/10 px-3 py-1 text-xs font-medium text-fuchsia-200">
            ...
            <p className="mt-4 max-w-2xl text-base leading-7 text-zinc-300 sm:text-lg">
              Pick up an existing conversation, meet a new companion, create a character,
              or start building toward scenes and adventures from one place.
            </p>
      ...
      <section className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <Metric
          ...
          icon={Coins}
        />
        <Metric label="Plan" value={account?.plan ?? "Browse"} icon={Sparkles} />
      </section>
          ...
          <div className="flex flex-wrap items-end justify-between gap-3">
            <div>
              ...
              <p className="text-sm text-zinc-400">
                Start here, then we will layer discovery and structured scenes on top.
              </p>
            </div>
            <a
              href="/companions"
        ...
        <aside className="space-y-4">
            ...
            <div className="mt-4 space-y-3">
              {recentConversations.length > 0 ? (
                recentConversations.map((conversation) => (
                  <a
                    key={conversation.id}
                    href={`/chat?companion=${encodeURIComponent(conversation.companion.slug)}`}
                  ...
                  >
                    ...
                    <div className="mt-1 line-clamp-2 text-xs leading-5 text-zinc-400">
                      {conversation.messages[0]?.content ?? conversation.companion.description}
                    </div>
          ...
          <div className="rounded-lg border border-zinc-800 bg-zinc-950 p-4">
            ...
            <div className="mt-4 grid gap-3">
              ...
              <ActionLink href="/discover" label="Discover Companions" icon={Compass} />
              <ActionLink href="/discover/saved" label="Saved Companions" icon={Bookmark} />
              <ActionLink href="/account/billing" label="Coins and Plan" icon={Coins} />
