...
export default async function SafeCustomizePage({
  ...
  if (!user) {
    redirect(
      `/login?next=${encodeURIComponent(`/companions/${slug}/customize`)}`,
    );
  }
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex-1">
          ...
          <div className="flex items-center gap-2 mt-1">
            ...
            {companion.age != null && (
              <span className="text-xs text-zinc-500">Age {companion.age}</span>
            )}
          </div>
        </div>
        <a
          href={`/rpg?companion=${companion.slug}`}
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-3">
              {companion.assets.map((a) => (
                <div
                  key={a.id}
                  className="overflow-hidden rounded-xl border border-zinc-800 aspect-[3/4] bg-zinc-900"
                >
                  {/* eslint-disable-next-line @next/next/no-img-element */}
                  <img
                    src={a.publicUrl ?? `/media/${a.id}`}
                    alt=""
