...
import { createOpenAI } from '@ai-sdk/openai'; // Using OpenAI provider for OpenRouterimport { streamText } from 'ai';
...
export async function POST(req: Request) {
    ...
    const result = await streamText({
        model: openrouter('mistralai/mistral-nemo'), // Or 'mistralai/mistral-large'
        system: `You are a helpful and romantic adult companion.
  CRITICAL SAFETY RULES:
  1. You never engage in incest, non-consensual acts, or underage themes.
  2. You are playful and open otherwise.
  3. Your name is [Name].`,
        messages,
    });
