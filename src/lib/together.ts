...
export const TOGETHER_CHAT_MODEL =
    process.env.TOGETHER_CHAT_MODEL ?? "Qwen/Qwen3.5-9B";

export const OPENROUTER_MODEL =
    process.env.OPENROUTER_MODEL ?? "deepseek/deepseek-v3.2";

export const OPENROUTER_FALLBACK =
    process.env.OPENROUTER_FALLBACK_MODEL ?? "@preset/noema-ai-fb";
...
export async function chatCompletion(params: ChatParams): Promise<OpenAI.Chat.ChatCompletion> {
    if (process.env.OPENROUTER_API_KEY) {
        ...
        try {
            ...
            console.warn(`[together] ${OPENROUTER_MODEL} failed (${(err as { status?: number })?.status ?? "unknown"}), trying ${OPENROUTER_FALLBACK}`);
        }
        try {
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            return await (client.chat.completions.create as any)({ ...params, model: OPENROUTER_FALLBACK, stream: false });
        } catch (err: unknown) {
            console.warn(`[together] fallback also failed (${(err as { status?: number })?.status ?? "unknown"})`);
        }
    }
