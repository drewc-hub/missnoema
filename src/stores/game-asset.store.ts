...
export const useGameAssetStore = create<GameAssetStore>((set, get) => ({
  ...
  audioMuted: typeof window !== "undefined" && localStorage.getItem("game-audio-muted") === "true",
  ...
  setAudioMuted: (muted) => {
    if (typeof window !== "undefined") localStorage.setItem("game-audio-muted", JSON.stringify(muted));
